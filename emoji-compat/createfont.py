...
FLATBUFFER_DEFAULT_MODULE_DIR = os.path.join(SCRIPT_DIR, '..', 'emoji-compat-flatbuffers')
# flatbuffer schema
FLATBUFFER_SCHEMA = os.path.join('data', 'emoji_metadata.fbs')
# file path for java header, it will be prepended to flatbuffer java files
FLATBUFFER_HEADER = os.path.join('data', 'flatbuffer_header.txt')
...
class EmojiFontCreator(object):
    ...
    def __init__(self, font_path, unicode_path, flatbuffer_path):
        validate_input_files(font_path, unicode_path, flatbuffer_path)
    ...
    def write_metadata_json(self, output_json_file_path):
        ...
        output_json['sourceSha'] = create_sha_from_source_files(
            [self.font_path, OUTPUT_META_FILE, os.path.join(self.flatbuffer_path, FLATBUFFER_SCHEMA)])
        output_json['list'] = []
    ...
    def create_font(self):
        ...
        with contextlib.closing(ttLib.TTFont(self.font_path, recalcTimestamp=False)) as ttf:
            ...
            flatc_command = ['flatc',
                             ...
                             '-b',
                             '-j',
                             os.path.join(self.flatbuffer_path, FLATBUFFER_SCHEMA),
                             output_json_file]
            ...
            update_flatbuffer_java_files(flatbuffer_java_dir, #tmp dir
                                         os.path.join(self.flatbuffer_path, FLATBUFFER_HEADER),
                                         os.path.join(self.flatbuffer_path, FLATBUFFER_JAVA_TARGET)) #target dir
...
def parse_args(argv):
    ...
    if len(argv) < 3:
        ...
        sys.exit(1)
    if len(argv) < 4:
        flatbuffer_dir = FLATBUFFER_DEFAULT_MODULE_DIR
    else:
        flatbuffer_dir = sys.argv[3]
    return (sys.argv[1], sys.argv[2], flatbuffer_dir)

def main():
    font_file, unicode_dir, flatbuffer_dir = parse_args(sys.argv)
    EmojiFontCreator(font_file, unicode_dir, flatbuffer_dir).create_font()
